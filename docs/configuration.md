...
Each new tab or split requests a new pane. Jasper captures its configured shell, arguments, environment overrides, scrollback capacity and shell-integration mode before dispatching the background launch. A reload cannot change an already queued request. On macOS, the account login shell is looked up on the launch worker at each start; the pane label and integration scripts then follow the resolved command. Existing sessions retain those settings. Pending views still receive the latest live font and behavior settings when ready.

With an empty program, macOS uses the current account login shell followed by `-l`, so changing the system default applies to the next tab, split or window without restarting Jasper. This lookup also refreshes the child’s `SHELL` before environment overrides, including when an explicit program such as tmux is configured. If the account lookup is unavailable or times out after two seconds, Jasper falls back to its inherited `SHELL`, then `/bin/zsh`. Linux uses inherited `SHELL` followed by `-l`, falling back to `/bin/bash` when unavailable or blank. Windows uses `powershell.exe -NoLogo`. Configured arguments append to that default command. An explicit program gets only the configured arguments, with no implicit login flag. Jasper does not split strings, expand shell expressions or parse a shell command line.
