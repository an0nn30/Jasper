...
| `workspace` | EDT windows, tabs, splits, panes, panel regions and the rail, action/config adapters, activity events and each window's presence in the terminal registry. WindowContent closes subscriptions/panes; each pane closes its session. |
