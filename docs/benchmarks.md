...
Payloads are deterministic (seed 42), ANSI-colored UTF-8 with a non-ASCII block character, staged before launch/timing. `stagedBytesPerPane` is `Files.size`, not Java string length. The number of staged bytes excludes the two timing markers and PTY newline expansion. Decimal MB/s means bytes / 1,000,000 / seconds; MiB is only used for the workload target.
