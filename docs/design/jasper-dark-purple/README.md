...
`jasper-dark-purple` is the default app and terminal palette, paired with the
existing `jasper-light` palette under Follow System. The Eclipse app icon supplies
...
[Complete terminal palette](../../examples/themes/jasper-dark-purple.toml)
...
selecting only `theme = "jasper-dark-purple"` also selects dark appearance, matching
the existing built-in-selector behavior.

`jasper-dark` remains available with its original terminal and chrome colors;
`jasper-light` is unchanged. Existing explicit theme selections are not rewritten.
Custom files keep their documented fixed classic `jasper-dark` inheritance for
...
- Terminal definition: `Palette.jasperDarkPurple()`; standalone terminal defaults
  and app config defaults use the new palette.
- Chrome definition: `JasperDarkPurpleLaf.properties`, layered over FlatDarkLaf.
...
```bash
./gradlew :jasper-app:mockUiPreview --args="/absolute/output/path --palette"
```
