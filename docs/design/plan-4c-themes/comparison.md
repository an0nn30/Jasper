...
> Historical evidence from 2026-09-12. Custom theme files and Follow System have since been removed; current [theme settings](../../configuration.md#theme-variant) support the bundled dark/light variants only.
...
The original `SystemThemePreview` fixture and its custom-theme API have been retired.
The adjacent `preview.gradle` is an archived recipe for that revision and cannot
run against the current classes. For current bundled-theme renders, run
`./gradlew :jasper-app:mockUiPreview` from the repository root; it writes the main
`docs/design/mock-ui-*.png` files and does not reproduce this historical custom variant.
