...
Both dark and light actual component previews were inspected during iteration. Focused tests verify continuous surfaces, row dimensions, padding and application font reset, live palette behavior, status clipping/running state, toolbar compact/hidden modes, accessible disabled actions, and the sampled title separator. The implementer full headless check passed 308 tests (72 app + 236 terminal), zero failures/errors, one known font skip. The final title-separator refinement subsequently passed all nine focused MockUi/MacTitleBar tests and regenerated both previews; the root review performs the final fresh whole-branch check.
