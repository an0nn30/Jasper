...
> Historical measurements from the compact-tab revision. The later [title-bar design](iterm-title-bar-reference.md) replaces the fixed-width tabs/sliding underline, and [compact padding](compact-padding-comparison.md) replaces the 24px inset. Running today's preview renders today's code, not the geometry recorded below.
