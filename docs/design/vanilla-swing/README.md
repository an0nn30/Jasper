...
Worktree: `.worktrees/vanilla-swing`, branch `codex/vanilla-swing`, based on committed main `a42c20f`. The main checkout's pending split-divider edits and the separate rail/vault work were not included or modified.

The user requested default Java Swing controls and colors, a normal OS title bar, standard content-area tabs, GNOME 2 icons, and a white-on-black terminal. `ui.laf` now chooses the Java built-in look and feel, with Metal as the default. The [configuration guide](../../configuration.md#swing-look-and-feel) and [copyable example](../../../config.example.toml) list every supported selector and platform availability.
...
```sh
./gradlew :moray-app:mockUiPreview --args='/absolute/output --palette --splits'
./gradlew :moray-app:commandPalettePreview --args='/absolute/output/commands'
```
...
The bundled OldGnome2 files retain their original pixels. [Provenance](../../../moray-app/src/main/resources/dev/moray/app/icons/oldgnome2/README.md) records their source and the absence of a license document in the supplied archive.
