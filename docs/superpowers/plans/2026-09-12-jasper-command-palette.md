...
**Spec:** `docs/superpowers/specs/2026-09-12-jasper-command-palette-design.md` (approved, including visual review).
...
- Modules: `jasper-terminal` and `jasper-app`; all palette code belongs in `dev.jasper.app`.
...
| Existing `WindowContent`, `TerminalWindow`, `JasperApplication`, `AppDirs`, `ActionId`, `KeyBindings`, `WindowChrome` | Narrow integration at existing ownership boundaries |

All paths above are under `jasper-app/src/main/java/dev/jasper/app/`. Tests use the matching package under `jasper-app/src/test/java/`. Do not introduce a production test-facade or a generic UI framework. Preserve existing constructors by delegating to new full constructors with in-memory history; only the production application path enables disk persistence.
...
- [x] **Run RED:** `./gradlew :jasper-app:test --tests '*CommandRegistryTest' --tests '*CommandSearchTest'`. Missing production types are expected initially; after the types exist, new regression tests must fail behaviorally before changes.
...
record Command(String id, Action action, List<String> keywords) {
    static final String TITLE = "jasper.command.title";
    static final String ICON = "jasper.command.icon";
    Command {
...
- [x] **Run GREEN:** the focused command above, then `./gradlew :jasper-app:test`. Review and commit `feat: add command registration and ranked search` with the required trailer.
...
- [x] **Run RED:** `./gradlew :jasper-app:test --tests '*CommandHistory*Test'`.
...
final class CommandHistory implements AutoCloseable {
    ...
    CommandHistory(Path file) {
        this(file, Executors.newSingleThreadExecutor(
            Thread.ofPlatform().daemon().name("jasper-command-history").factory()), SwingUtilities::invokeLater);
    }
...
- [x] **Run GREEN:** `./gradlew :jasper-app:test --tests '*CommandHistory*Test' --tests '*AppDirsTest'`, then app tests. Review and commit `feat: persist shared command palette recents` with trailer.

### Task 3: Build the approved headless-testable palette component

**Files:** Create `CommandPalette.java`, `CommandPaletteTest.java`; modify shared/per-theme FlatLaf properties under `jasper-app/src/main/resources/dev/jasper/app/themes/` only for semantic palette keys required below.
...
- [x] **Run RED:** `./gradlew :jasper-app:test --tests '*CommandPaletteTest'`.
...
```properties
Jasper.paletteBackground = $Jasper.tabSelectedBackground
Jasper.paletteForeground = $Jasper.chromeForeground
Jasper.paletteMutedForeground = $Jasper.mutedForeground
Jasper.paletteBorder = $Component.borderColor
Jasper.paletteAccent = $Component.focusedBorderColor
Jasper.paletteSelectionBackground = @selectionBackground
Jasper.paletteSelectionForeground = @selectionForeground
```
...
**Files:** Create `WindowCommands.java`, `WindowCommandPalette.java`, `WindowCommandPaletteTest.java`, `WindowCommandsTest.java`; modify `WindowContent.java`, `WindowChrome.java`, `TerminalWindow.java`, `JasperApplication.java`.
...
- [x] **Run RED:** `./gradlew :jasper-app:test --tests '*WindowCommand*Test'`.
...
- [x] **Run RED:** `./gradlew :jasper-app:test --tests '*CommandPaletteShortcutsTest' --tests '*PaletteKeyRouterTest'`.
...
**Files:** Create `CommandPalettePreview.java` and `CommandSearchMeasurement.java` in the app test source set; add explicitly opt-in headless JavaExec tasks in `jasper-app/build.gradle.kts`; create `docs/design/command-palette/README.md`, rendered PNGs, `docs/command-palette.md` and `docs/superpowers/plans/2026-09-12-jasper-command-palette-manual-check.md`. Update `docs/configuration.md`, `config.example.toml`, `docs/STATUS.md`, the binding Phase 1 spec amendment and this plan's status/checklists.

**Interfaces:** `:jasper-app:commandPalettePreview --args="<output-dir>"` writes actual Swing renders without creating JFrame or login shells. `:jasper-app:commandSearchMeasurement --args="<output-dir>"` writes timing/allocation results using a deterministic catalog. Neither runs under `check` or opens a window.
...
Recents are shared across windows and saved in command-history.toml under Jasper's
