# Jasper Plan 3.5 — macOS Chrome, Themes and Toolbar
...
**Parent:** [Phase 1 design](../specs/2026-09-10-jasper-phase-1-terminal-design.md), especially §§5–7. This amendment supersedes the earlier sequencing that deferred all theme work to Plan 4 and the assumption that the current toolbar presentation is final.

**Execution update:** The user approved work and selected toolbar study B (fuller, two-tone colored icons). The [concrete design](../specs/2026-09-11-jasper-plan-3-5-titlebar-themes-design.md) and [four-task implementation plan](2026-09-11-jasper-plan-3-5-titlebar-themes-implementation.md) now cover title bar, coordinated themes and that toolbar treatment together. Initial title layout uses a slim custom surface with separate tabs; an optional integrated-tab preference remains open to user steering.
...
Use Atom/One Dark and One Light as the visual direction, not an exact-clone requirement or a final approved palette. Retain the built-in identifiers `jasper-dark` and `jasper-light`; settle exact colors through visual review.
