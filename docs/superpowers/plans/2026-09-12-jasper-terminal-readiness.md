...
**Spec:** `docs/superpowers/specs/2026-09-12-jasper-terminal-readiness-design.md` plus the parent Phase 1 memory amendment.

**Status:** All local tasks complete. Task reviews approved through `10cc444`; whole-branch review of `d3b5d11..5f4f804` found one hidden-search gap, fixed in `8d84e7d` and approved by scoped re-review with no remaining findings. Final forced build/package: 576 tests, 575 passed, one known skip. Repeated memory results retain runtime `10cc444`; fresh final-code throughput and a native matrix smoke passed. The branch, based on `d3b5d11`, was published with user approval and then merged locally into main. Merged-check test-readiness fix `7481114` preserves the exact URL assertion and waits for complete wrapped input. Fresh merged check: 576 tests, 575 passed, one known skip, all eight tasks executed. Existing main files remain preserved. Artifacts are archived under main `build/readiness-2026-09-12/`; the completed local branch/worktree are cleaned up. Main and the integration follow-up have not been pushed. [Evidence, execution deviations and CI results and pending native/trial gates](../../terminal-readiness.md). Test-only Windows portability follow-ups `a5f4dc1` and `c793925` passed [three-platform CI run](https://github.com/an0nn30/moray/actions/runs/34724855954); production runtime is unchanged. The two-week trial has not started.

## Global Constraints

- Java 25 JetBrains toolchain; use the Gradle wrapper.
- `jasper-terminal` never depends on `jasper-app`; no public JediTerm types.
...
try (var log = AppLog.open(tempDir)) {
    System.getLogger("dev.jasper.terminal.Test").log(System.Logger.Level.WARNING,
...
**Files:** Modify `jasper-app/src/main/java/dev/jasper/app/Bench.java`, `jasper-app/build.gradle.kts`; create focused benchmark classes under `dev.jasper.app` and tests, `docs/benchmarks.md`, and platform runner scripts under `tools/benchmarks/` if useful. Minimal package-private application accessors or constructor injection are allowed only to drive real app components safely. Add no benchmark controls to the normal UI and no default startup cost. Runtime baseline source is the current packaged classes.

**Interfaces:** Keep `:jasper-app:bench` opt-in. Add `:jasper-app:memoryBench` opt-in; provide equivalent commands using the generated package's bundled Java/classpath. Structured result file includes schema/version, environment metadata, scenario/sample timestamps, units/sources, nullable unavailable metrics and completion/failure status. Fixture child is argument-safe on Mac/Windows, deterministic, bounded and does not start the user's login shell. Native runs require controller approval after active-game/VM constraint resolves; worker performs headless tests only.
...
- [x] Fresh `./gradlew build :jasper-app:packageDist --rerun-tasks`; count XML results, check source hygiene/diff and package integrity. Preserve final package in a clear output path.
...
- [x] Create one concise readiness report listing logging, hardening, memory, throughput, package, CI, Mac/Windows manual checks and the two-week trial (not started). Link exact evidence and runnable follow-up commands. Commit docs, request final whole-branch review, apply one combined fix wave and scoped re-review, rerun affected verification. Stop at the pre-trial handoff; do not start the two-week clock.
