...
**Spec:** docs/superpowers/specs/2026-09-11-jasper-plan-4a-config-design.md
...
- Supported settings only: window.tab_height(38,28–72), window.toolbar(icons_and_labels/icons/hidden), window.status_bar(true), font.size(16,finite6–72), colors.theme(jasper-dark/jasper-light), keybindings(existing platform parser/defaults).
...
**Files:** Create AppDirs.java, AppArguments.java, ConfigSnapshot.java, ConfigDiagnostic.java, ConfigLoader.java and focused tests under jasper-app/src/{main,test}/java/dev/jasper/app; modify jasper-app/build.gradle.kts for implementation("org.tomlj:tomlj:1.1.1").
...
assertThat(AppDirs.resolve("Mac OS X", Map.of(), Path.of("/home/test")).configFile())
    .isEqualTo(Path.of("/home/test/.config/jasper/config.toml"));
```
