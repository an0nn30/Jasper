# Jasper Plan 3 — App Chrome Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** A tabbed, split-pane, multi-window Jasper terminal with shared application actions, find UI and native desktop chrome.
...
**Spec:** `docs/superpowers/specs/2026-09-11-jasper-plan-3-app-chrome-design.md`; parent `docs/superpowers/specs/2026-09-10-jasper-phase-1-terminal-design.md`.
...
**Follow-up amendment:** The user has requested [Plan 3.5 — macOS chrome, themes and toolbar](2026-09-11-jasper-plan-3-5-chrome-and-themes.md) before Plan 4. This completed plan remains the implementation record; coordinated built-in themes and toolbar styling now have a separate follow-up milestone.

## Global Constraints

- Packages: `dev.jasper.terminal` and `dev.jasper.app`.
- Java 25 on JetBrains Runtime; preserve existing Gradle/JUnit/AssertJ versions.
- No public terminal method takes or returns a JediTerm type; jasper-terminal never depends on jasper-app.
...
**Files:**
- Create: `jasper-app/src/main/java/dev/jasper/app/{SplitTree,TabState,ActionId,KeyBindings}.java`
- Test: `jasper-app/src/test/java/dev/jasper/app/{SplitTreeTest,TabStateTest,KeyBindingsTest}.java`
...
Run `./gradlew :jasper-app:test --tests '*SplitTreeTest' --tests '*TabStateTest' --tests '*KeyBindingsTest'` and record missing-class failure.
...
**Files:**
- Modify: `jasper-terminal/src/main/java/dev/jasper/terminal/TerminalView.java`
- Modify: `jasper-terminal/src/main/java/dev/jasper/terminal/TerminalSession.java`
- Modify: `jasper-terminal/src/main/java/dev/jasper/terminal/PtyConnector.java` and `SessionDisplay.java` as needed for close and alternate-buffer callbacks.
- Test: `jasper-terminal/src/test/java/dev/jasper/terminal/TerminalAppIntegrationTest.java`
- Test: `jasper-terminal/src/test/java/dev/jasper/terminal/TerminalAppearanceTest.java`
...
Run `./gradlew :jasper-terminal:test --tests '*TerminalAppIntegrationTest' --tests '*TerminalAppearanceTest'` and record failure before implementation.
...
**Files:**
- Modify: `jasper-app/build.gradle.kts`, `jasper-app/src/main/java/dev/jasper/app/Main.java`
- Create: `jasper-app/src/main/java/dev/jasper/app/{JasperApplication,TerminalWindow,TerminalTab,TerminalPane,FindBar,AppIcons}.java`
- Create: `jasper-app/src/main/resources/dev/jasper/app/icons/*.svg` and `LICENSE.txt`
- Test: `jasper-app/src/test/java/dev/jasper/app/{FindBarTest,TerminalTabTest,ApplicationActionsTest}.java`; add focused lifecycle helpers/tests as needed.

**Interfaces:**
- Consumes Task 1 models and Task 2 terminal hooks; inspect their final report before implementation.
- `JasperApplication` owns all windows and shell-launch executor; `newWindow(Path)`, `quit()`.
