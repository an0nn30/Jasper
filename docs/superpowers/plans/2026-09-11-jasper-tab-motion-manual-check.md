...
Pending user execution. Run `./gradlew :jasper-app:run` from `/Users/dustin/projects/moray`. Agents do not launch the app unattended.
...
- [ ] Close an active tab, an inactive tab before the selection, and a tab still expanding. Departing tabs contract while the underline and following tabs ease toward their final positions. Closed controls stop accepting input immediately. Close several quickly, then hide/resize/reorder: no departed control should remain or keep animating. Closing the final tab closes the window immediately.
