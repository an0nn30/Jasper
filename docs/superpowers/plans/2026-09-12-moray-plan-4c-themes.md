# Moray Plan 4c — Themes Implementation Plan
...
**Spec:** [Plan 4c design](../specs/2026-09-12-moray-plan-4c-themes-design.md), amending the [Phase 1 design](../specs/2026-09-10-moray-phase-1-terminal-design.md).
...
- `moray-terminal` never depends on `moray-app`; no public JediTerm types, no new plugin/backend interfaces.
...
All new Java files use package `dev.moray.app` under `moray-app/src/main/java/dev/moray/app/`; test counterparts use `moray-app/src/test/java/dev/moray/app/`.
...
| `ThemeController.java`, `ConfigurationController.java`, `MorayApplication.java` | Resolve/apply saved, manual and system changes; own listener lifecycle |
...
| `docs/examples/themes/moray-custom.toml` | Complete copyable supported theme fixture |

Keep `BuiltinTheme` unchanged as the two FlatLaf choices and built-in palette factory. Replace `ConfigSnapshot`'s `BuiltinTheme theme` component with `ColorsConfig colors`; do not invent a `theme()` accessor that lies for custom themes. Migrate callers by compiler errors and `rg '\.theme\(\)|BuiltinTheme' moray-app/src`.
...
record ResolvedTheme(BuiltinTheme chrome, dev.moray.terminal.Palette palette) {}
record ThemeState(ColorsConfig saved, dev.moray.terminal.Palette loaded,
                  Appearance override, BuiltinTheme system) {
    static ThemeState defaults();
    ThemeState configure(ColorsConfig next, dev.moray.terminal.Palette palette);
...
`Appearance.java` contains the enum declared in contracts. `ConfigLoader` adds `appearance` to the colors field whitelist, changes its theme field to `String theme = "moray-dark"`, and adds `Appearance appearance = Appearance.SYSTEM`. Parse with existing positioned helpers:
...
@Test void parsesSupportedColorsAndKeepsFixedDefaults() {
    ...
    assertThat(result.palette().foreground()).isEqualTo(Palette.morayDark().foreground());
}
...
**Files:** Create `SystemAppearance.java`, `SystemAppearanceTest.java`; modify root `build.gradle.kts`, `moray-app/build.gradle.kts`. No native initialization in headless tests.
...
Use the upstream package spelling `jthemedetecor`, despite the artifact's different spelling. `production()` allocates a daemon single-thread executor named `moray-appearance`, passing `SwingUtilities::invokeLater`; `fixed` uses the same constructor with a no-op register/remove and a constant BooleanSupplier, so tests never call the production factory.
...
**Files:** Modify `ThemeController.java`, `ConfigurationController.java`, `MorayApplication.java`, `Main.java`, `WindowContent.java`, `TerminalPane.java`, `WindowChrome.java`, `WindowStatusBar.java`, `MacTitleBar.java`, `TerminalWindow.java`, `ConfigTemplate.java`, root `config.example.toml`, `docs/configuration.md`, `README.md`, `docs/STATUS.md`. Extend `ThemeControllerTest.java`, `ConfigurationControllerTest.java`, `ConfigurationStatusTest.java`, `ConfigTemplateTest.java`, `MockUiTest.java`, `MacTitleBarTest.java`; create `SystemThemeIntegrationTest.java` and `docs/superpowers/plans/2026-09-12-moray-plan-4c-manual-check.md`. Remove the Task 1 temporary bridge.
...
// MorayApplication(ConfigService service, SystemAppearance source) is used by Main;
