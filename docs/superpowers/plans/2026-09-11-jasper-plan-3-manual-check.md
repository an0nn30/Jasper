...
1. Launch from `/Users/dustin/projects/moray` on main with `./gradlew :jasper-app:run`. Confirm shell prompt, SVG toolbar, tab strip, status.
...
12. When no game or VM is running, benchmark separately ./gradlew :jasper-app:bench; record actual MB/s.
