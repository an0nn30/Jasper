...
**Status:** Implemented on `claude/shell-integration`, commits `38e511b..this commit`. Amended after the whole-branch review; the amendments are marked **(review)** below.
...
- **(review)** never writes through a shell function the user may have redefined: `printf` is called as a builtin and `base64`/`tr` through `command`; reads every variable it tests unset-safely, so `set -u` / `setopt nounset` stays quiet; and skips the `PS1`/`PROMPT`/`PROMPT_COMMAND` assignment when the parameter is readonly rather than failing on every prompt;
...
- **bash** targets bash 3.2 (macOS `/bin/bash`) upward. `PROMPT_COMMAND` gets a status-capture function first and the prompt function last, honouring both the string form and the array form (bash 5.1+). The `DEBUG` trap chains any existing trap, ignores its own functions and `PROMPT_COMMAND`, and reads the command line from `history 1`; when the line was not added to history it falls back to `BASH_COMMAND`, detected by the history number not having advanced since the prompt. **(review)** The one exception is `HISTCONTROL` containing `ignorespace` or `ignoreboth`: the user deliberately hid that line, so bash emits neither the `cmd` payload nor **C** — emitting C alone would let the screen read recapture it. **D** still fires, so the prompt cycle is intact and only the command text is lost.
- **fish** targets fish 3.x and 4.x. `fish_prompt` event handler emits D (using the status recorded by a `fish_postexec` handler), OSC 7 and A; `fish_preexec` emits `cmd` and C; the original `fish_prompt` function is copied and wrapped so B follows its output. **(review)** The wrap happens inside the prompt event with a marker check, not once at load: fish sources `vendor_conf.d` *before* `config.fish`, so a prompt defined there (starship, Tide, oh-my-fish) would otherwise replace the wrapper for the life of the session. fish 4 emits some marks itself; duplicates are harmless because Jasper deduplicates a repeated A on the same row and ignores a C without a pending B.
...
**(review)** Four corrections to the table above. Jasper's own `JASPER_*` markers are scrubbed from the inherited environment before anything is added, because the scripts export `JASPER_INTEGRATION_LOADED` and return early when they see it — without the scrub, a Jasper launched from an integrated pane gets no marks in any pane. bash injection is skipped entirely when the arguments contain `--norc`, `--rcfile`, `--init-file` or any form of `-c`, since bash would ignore Jasper's rc file while `-l` had already been stripped; `-l` inside a short cluster (`-il`) counts as a login flag, and a `-l` past `--` or an operand is the command's own argument and is left alone; `--noprofile` suppresses `JASPER_LOGIN_SHELL`, which `rc.bash` reads as permission to source the very files `--noprofile` refused. `XDG_DATA_DIRS` is not prefixed again when the Jasper entry is already first. Finally `JASPER_SHELL_INTEGRATION` is exported *before* the `[terminal.env]` overlay and injection follows the effective value, so the documented "a `[terminal.env]` override wins" rule holds for it as it does for `TERM_PROGRAM`.

## Jasper's receiving side

`TerminalSession.onCustomCommand` learns `cmd`: it base64-decodes the payload as UTF-8 and keeps it as the command text for the current cycle. At C, that text replaces the screen read; the screen read remains the fallback when no `cmd` arrived. At D, or at the next A, the listener fires as today. A malformed payload is ignored. **(review)** "Malformed" includes a payload over 16 KiB — the same bound `ShellHistoryParser` puts on the lines it reads from disk — and valid base64 whose bytes are not valid UTF-8, which was previously substituted to U+FFFD and reported as the command; the payload is no longer trimmed, because it is the exact command line. The text is discarded at D, at RIS and on an alternate-screen switch, so an unconsumed payload cannot be reported as the next command. A repeated A on the row Jasper already marked does not flush the cycle, so a shell that emits its own A (fish 4) no longer loses the exit status. `TerminalSession.shellIntegrationDetected()` becomes true at the first A mark and notifies listeners when it flips, because the mark itself draws nothing; `TerminalPane` exposes it and the status bar shows a filled dot after the shell name when true and a hollow one otherwise, with the tooltip "Shell integration active" / "Shell integration not detected".
...
- **(review)** The real-shell script tests run the shells on pipes and assert over the raw byte stream; `ShellIntegrationEndToEndTest` covers the session path the list below describes, driving a real zsh through `LaunchSettings` and `TerminalSession.start` and asserting through the listener. Prompt rows stay covered by the terminal module's own tests, since `promptRows()` is package-private and widening it for a test would breach the module's API discipline.
