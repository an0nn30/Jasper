...
Use the JDK logging facility rather than introducing a logging dependency. Terminal code uses `System.getLogger` under `dev.moray.terminal`; the application installs a `java.util.logging` handler for the `dev.moray` namespace. The existing SLF4J no-op binding for third-party libraries remains intentional: this change covers Moray-owned diagnostics.
