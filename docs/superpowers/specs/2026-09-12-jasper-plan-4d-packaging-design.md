...
**Status:** Approved by the user on 2026-09-12; implemented and reviewed through `de4ec8c`. Final forced full-suite/package verification passed. Integrated locally on main after user approval, with README build instructions and a freshly verified main-checkout DMG. Native desktop/Windows execution remains pending. See [verification](../plans/2026-09-12-jasper-plan-4d-verification.md).
...
Use Gradle tasks that invoke the selected JetBrains Runtime 25 SDK's `jpackage`. Build native packages on their target operating system and CPU architecture; do not attempt cross-compilation. Users of packaged Jasper do not need a separate Java installation.

Proposed first outputs:

| Host | Application image | Distribution |
|---|---|---|
| macOS Apple Silicon or Intel, with matching JBR SDK | `Jasper.app` | `.dmg` containing the application |
| Windows x64, with matching JBR SDK | `Jasper/`, including `Jasper.exe` and its runtime | Portable ZIP containing the whole `Jasper/` directory |
...
Add the packaging task definitions in a focused Gradle script applied by `jasper-app/build.gradle.kts`. Keep the existing two Java modules and avoid a new runtime abstraction or installer framework.

- `:jasper-app:packageApp` creates the native application image on the current supported host.
- `:jasper-app:packageDist` creates the host distribution from that image.
- `:jasper-app:verifyPackage` checks the packaged contents and launcher configuration without starting the desktop or a shell.
...
Use the same Java 25 JetBrains toolchain for compilation, runtime generation and jpackage. Resolve `jpackage` from that installation instead of the shell's PATH. Retain all existing application JVM arguments, especially `--enable-native-access=ALL-UNNAMED` and the macOS application name. The entry point remains `dev.jasper.app.Main`.
...
Metadata: application name `Jasper`, initial package version `1.0.0`, macOS identifier `dev.jasper.app`. Permit `-PjasperVersion=X.Y.Z`; reject values incompatible with native package versions before calling jpackage. Include OS and architecture in distribution filenames. Put all generated staging, application images and distributions under `jasper-app/build/packaging/`.
