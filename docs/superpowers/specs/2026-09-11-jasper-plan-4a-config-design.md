...
`AppDirs` resolves supplied OS/environment/home inputs: macOS `~/.config/jasper`, Linux `$XDG_CONFIG_HOME/jasper` or `~/.config/jasper`, Windows `%APPDATA%/jasper` (fallback home/AppData/Roaming/jasper). It exposes config.toml, themes and logs paths without creating them. Relative XDG roots are ignored as invalid; normalize explicit CLI paths to absolute. `--config <path>` overrides only config file location; `--help` prints usage without starting Swing. Invalid/missing/duplicate CLI options fail clearly without opening windows.
...
| colors.theme | jasper-dark | jasper-dark or jasper-light; live existing theme controller |
