...
**As of:** 2026-09-12. Plan 4b and the compact padding/copyable config follow-up are integrated on `main` at `8ad5deb`. Fresh merged verification executed all eight Gradle tasks: 443 tests, 442 passed and one known font skip, with no failures/errors. Continue from `/Users/dustin/projects/moray`. The root [config.example.toml](../config.example.toml) is ready to install; see [configuration](configuration.md) for details. Native UI, benchmark and daily-use acceptance remain open.

**Shell-exit follow-up:** `terminal.on_exit` now supports `keep_open` (the default), `close_on_success` and `close`. Policy reloads apply to future exits in existing and pending panes without changing view options; retained output stays open. Closure follows the existing pane → empty tab → empty window lifecycle and preserves siblings. This bounded follow-up uses implementer TDD/self-review plus independent root review, with no new architecture or historical-plan rewrite. Fresh `./gradlew check --rerun-tasks` executed all eight tasks: 463 tests, 462 passed, one known font skip, no failures/errors. Source hygiene and diff checks passed. Independent review and integration are pending; [native shell-exit checks](superpowers/plans/2026-09-12-moray-plan-4b-manual-check.md#shell-exit-follow-up) remain user-run. Next Plan 4 work remains custom themes/system appearance, logging/launcher cleanup and packaging, then acceptance and daily use before SSH.
