...
**As of:** 2026-09-12. Plan 4b and its padding/config-example/shell-exit follow-ups are integrated on `main` at `f1fd02a`. Fresh merged verification executed all eight Gradle tasks: 463 tests, 462 passed and one known font skip, with no failures/errors. Continue from `/Users/dustin/projects/moray`. The root [config.example.toml](../config.example.toml) includes `terminal.on_exit`; see [configuration](configuration.md) for usage. Native UI, benchmark and daily-use acceptance remain open.
