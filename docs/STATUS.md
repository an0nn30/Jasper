...
**Vanilla Swing appearance (2026-09-14):** Implemented in `.worktrees/vanilla-swing` on `codex/vanilla-swing`, based on committed main `a42c20f`. Standard Swing controls, normal OS title bar, content-area tabs and supplied OldGnome2 icons replace FlatLaf/custom title chrome. `ui.laf` selects Metal (default), Nimbus, Motif, system, Aqua, Windows, Windows Classic or GTK; unsupported choices fall back to Metal with a config warning. Terminal defaults stay white on black with classic ANSI colors. Live LAF reload preserves sessions, hidden/zoomed panes, split ratios and existing actions. macOS text editing shortcuts are preserved across built-ins. Legacy theme/tab-height keys warn; theme files are no longer read or watched. The visual amendment supersedes previous chrome/theme/motion requirements; this bounded follow-up used independent config/control implementation and review without a new plan document. Review's native-tab-padding focus and middle-click findings were reproduced and fixed; scoped re-review has no remaining findings. Fresh `./gradlew check --rerun-tasks` passed all eight tasks: 598 tests, 597 passed, 1 existing font skip, no failures/errors. Metal/Nimbus/Motif headless window and palette renders were inspected. [Previews, reproducible commands and native acceptance limits](design/vanilla-swing/README.md), [configuration](configuration.md#swing-look-and-feel). Main's uncommitted split-divider changes and separate rail/vault work remain untouched. Changes are uncommitted in the worktree; no GUI, packaging, merge or push.
