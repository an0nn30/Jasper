...
**Plan 3 execution update:** Tasks 1 and 2 are implemented and reviewed on `codex/plan-3-app-chrome`; Task 3 desktop integration is underway. Models/keybindings landed at `a75d5fa`; terminal APIs and carryovers at `56b251d`, with reviewed reparenting fixes at `73f105c`. Latest terminal check: 229 tests, 0 failures/errors, 1 expected font skip. The view retains a bounded search executor across reparenting and reconciles missed row resets by session epoch. Other uncovered performance/selection findings remain for follow-up hardening. GUI and final benchmark acceptance are user-run under AGENTS.md. Plan 3 exposes disabled Settings/Reload controls and Built-in defaults status; TOML remains Plan 4. Non-mac explicit cmd+shift defaults add Alt to avoid collisions; macOS bindings are unchanged.
