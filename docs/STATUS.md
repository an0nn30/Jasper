...
**Plan 4d preparation:** The user requested macOS and Windows packaging with `jpackage`, with Windows desktop acceptance performed manually. [Proposed packaging design](superpowers/specs/2026-09-12-moray-plan-4d-packaging-design.md) is ready for review on `codex/plan-4d-packaging` in `.worktrees/plan-4d-packaging`; implementation has not begun. It advances packaging and the necessary launcher-environment cleanup ahead of broader app logging. Proposed outputs are macOS `.app`/`.dmg` and a portable Windows ZIP, pending the user's format preference. `origin` is now configured at `https://github.com/an0nn30/moray.git`; remote access succeeded and it contains no refs. No push was performed. Fresh worktree `./gradlew check` ran all eight tasks: 506 tests, 505 passed, one known skip, no failures/errors. This preparation does not change runtime code.
