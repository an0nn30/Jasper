...
**Current readiness work (2026-09-12):** Logging, terminal hardening and repeated packaged memory/throughput measurements are complete on `codex/terminal-readiness` in `.worktrees/terminal-memory-plan`; main remains `d3b5d11`. All task reviews approved. Whole-branch review found one hidden-search lifecycle gap, resolved by `8d84e7d` and approved in scoped re-review; no findings remain. Fresh final build/package: 576 tests, 575 passed, one known skip; native image and DMG verification passed. The pre-review-fix stress comparison (`10cc444`) showed 10.0% less allocation and 2.9% lower peak RSS, with idle RSS unchanged. Fresh final-code throughput median is 37.05 MB/s; all three runs passed 35 MB/s, while the 45 MB/s target remains unmet. Full memory results retain their measured revision. [Readiness handoff, evidence, execution decisions and remaining gates](terminal-readiness.md). Publication is authorized and complete; [three-platform CI run](https://github.com/an0nn30/moray/actions/runs/34724855954) passed on all three platforms after test-only portability fixes `a5f4dc1` and `c793925`. Normal Mac/Windows native acceptance and the two-week trial remain pending. No merge or trial start is implied. Preserve local benchmark artifacts and DMG before cleaning/removing this worktree.
...
3. [three-platform CI run](https://github.com/an0nn30/moray/actions/runs/34724855954) passed macOS, Ubuntu and Windows at `c793925`. Windows ConPTY/packaged desktop and Linux desktop/font behavior still require native acceptance. The branch is published; no merge is authorized.
...
The fully reviewed readiness branch resolves the carried-over gesture ownership, bounded logical-line traversal, idle frame/blink work, pure mouse-report snapshots, visible search scanning, EDT browser dispatch, multi-notch wheel and selection integrity findings. See the [handoff](terminal-readiness.md) for evidence. Remaining limitations:
