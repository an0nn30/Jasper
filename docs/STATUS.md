...
**Follow-up in progress:** The user requested a shorter configurable tab/title row, quick eased tab-entry/underline animations, and Cmd/Ctrl tab navigation. [Design](superpowers/specs/2026-09-11-moray-tab-motion-design.md) and [plan](superpowers/plans/2026-09-11-moray-tab-motion.md) supersede the fixed 54px mock height. Work continues on the same unmerged branch. Default 38px with a live current-window View control; persistence remains Plan 4. Height and shortcut implementation is task-reviewed (`0c4ab43`), with 80 app tests passing; animation work is active. Existing shortcut engine is extended, not replaced. Earlier check/review totals below describe the completed screenshot revision, not this ongoing follow-up.
