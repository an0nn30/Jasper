...
**As of:** 2026-09-12. Plan 4c is implemented and all four task reviews are complete through `fd6811b` on `codex/plan-4c-themes`; whole-branch review is next. Continue from `/Users/dustin/projects/moray/.worktrees/plan-4c` for this feature. Fresh headless verification: 502 tests, 501 passed, one known font skip, no failures/errors. Main remains at the prior Plan 4b/shell-exit baseline with the user's local example-config edit preserved. Native UI, benchmark and daily-use acceptance remain open.
