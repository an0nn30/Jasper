...
**Plan 3 execution update:** Task 1 models/keybindings implemented at `a75d5fa`, under review. Task 2 now includes lost link-gesture reset, alternate-screen selection/search invalidation, prompt pruning and reliable child termination. Other uncovered performance/selection findings remain for a follow-up hardening pass. GUI and benchmark acceptance are user-run under AGENTS.md. Plan 3 exposes disabled Settings/Reload controls and Built-in defaults status; TOML remains Plan 4. Non-mac explicit cmd+shift defaults add Alt to avoid collisions; macOS bindings are unchanged.
