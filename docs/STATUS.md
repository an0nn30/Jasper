...
Task 1 was independently approved with no required fixes. Task 2 added RED/GREEN environment regressions; full `./gradlew check` passed with 508 tests, 507 passed and one known font skip. Final branch/package verification will be recorded after whole-branch review. One intermediate forced run observed a concurrent RIS snapshot NPE in `TerminalLiveOptionsTest` in unchanged terminal source/tests; targeted and subsequent full runs passed. Keep this as a test-reliability follow-up, distinct from the known font skip. No claim of Windows or desktop runtime acceptance is made.
