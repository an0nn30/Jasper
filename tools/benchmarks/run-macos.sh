...
set -eu
if [ "$#" -lt 4 ]; then
    echo "Usage: $0 /absolute/Moray.app throughput|memory REVISION /absolute/result.json [benchmark options...]" >&2
    exit 2
fi
image=$1
mode=$2
...
exec "$image/Contents/runtime/Contents/Home/bin/java" --enable-native-access=ALL-UNNAMED \
    -Dapple.awt.application.name='Moray benchmark' -cp "$image/Contents/app/*" \
    "dev.moray.app.$entry" --revision "$revision" --output "$output" "$@"
