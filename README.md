...
`ui.theme.style` selects `"modern"` (default) or [Retro Metal](docs/configuration.md#retro-metal-appearance); changing it requires fully quitting and relaunching Jasper. In modern mode, `ui.theme.variant` picks the bundled dark (default) or light theme and reloads live. Retro uses light Metal controls, classic icons and a high-contrast terminal. `[ui.font]` sets UI typography independently of terminal `[font]`; its family and optional 8–32-point size update live. See [theme configuration](docs/configuration.md#theme-variant). Native macOS and Windows packaging commands and acceptance checks are in the [packaging guide](docs/packaging.md). Bounded application logs and their privacy rules are described in [application diagnostics](docs/diagnostics.md). Native font/input/audio and initial sizing acceptance is tracked in the [terminal configuration checklist](docs/superpowers/plans/2026-09-12-jasper-plan-4b-manual-check.md).
