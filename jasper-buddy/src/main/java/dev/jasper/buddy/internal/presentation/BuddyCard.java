...
final class BuddyCard {
    ...
    static Font titleFont(BuddyOptions options) {
        return options.primaryFont().deriveFont(Font.BOLD, 13f);
    }

    static Font detailFont(BuddyOptions options) { return options.primaryFont().deriveFont(Font.PLAIN, 13f); }

    static int height(JComponent owner) { return HEIGHT; }

    static Color detailColor(BuddyOptions options) { return options.dark() ? new Color(171, 171, 171) : new Color(109, 109, 109); }
    ...
    static void paint(BuddyOptions options, Graphics2D g2, JComponent owner, BuddyNotice notice, Rectangle bounds,
                      float hover, float opacity, boolean dismissable, int count) {
        paint(options, g2, owner, notice, bounds, hover, opacity, dismissable, count, System.nanoTime());
    }
    ...
    static void paint(BuddyOptions options, Graphics2D g2, JComponent owner, BuddyNotice notice, Rectangle bounds,
        ...
        try {
            ...
            if (count > 0) {
                c.setFont(detailFont(options));
                String badge = Integer.toString(count);
                c.setColor(detailColor(options));
            ...
            if (notice.wantsAttention()) {
                paintGlyph(options, c, notice.state(), right - GLYPH_SIZE, (bounds.height - GLYPH_SIZE) / 2);
            ...
            try {
                ...
                text.setFont(titleFont(options));
                text.setColor(notice.orphaned() ? detailColor(options) : dark ? Color.WHITE : new Color(30, 30, 30));
                paintText(text, notice.title(), right, 25, hover);
                text.setFont(detailFont(options));
                text.setColor(detailColor(options));
    ...
    private static void paintGlyph(BuddyOptions options, Graphics2D g2, BuddyNotice.State state, int x, int y) {
        ...
        try {
            c.setColor(good || bad ? (configured != null ? configured
                : good ? new Color(0x5AB07A) : new Color(0xC45A53)) : detailColor(options));
