...
final class BuddyBubblePanel extends JComponent {
    ...
    BuddyBubblePanel(BuddyOptions options) {
        this.options = java.util.Objects.requireNonNull(options);
        setOpaque(false);
    }

    Font titleFont(BuddyBubbleContent.Style style) {
    ...
    Font detailFont() { return font(Font.PLAIN, 14f); }

    private Font font(int style, float size) {
        return options.primaryFont().deriveFont(style, size);
    }
