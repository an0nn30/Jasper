...
class BuddyShimmerTest {
    private BufferedImage paint(dev.jasper.buddy.config.BuddyOptions options, BuddyNotice.State state, long time) {
        var image = new BufferedImage(330, 56, BufferedImage.TYPE_INT_ARGB);
        var graphics = image.createGraphics();
        try {
            BuddyCard.paint(options, graphics, new JPanel(), new BuddyNotice(new BuddyNoticeId("test", "one"), BuddyNotice.Kind.TASK, "Title stays still", state, () -> "Thinking about the next command", () -> {}),
    ...
    @Test void highlightTravelsAcrossDetailOnlyAndStopsWhenTheCommandFinishes() throws Exception {
        BuddyTestAppearance.edt(() -> {
            for (boolean dark : new boolean[]{false, true}) {
                ...
                BufferedImage rest = paint(options, BuddyNotice.State.RUNNING, 2_100_000_000L);
                BufferedImage early = paint(options, BuddyNotice.State.RUNNING, 400_000_000L);
                BufferedImage late = paint(options, BuddyNotice.State.RUNNING, 1_100_000_000L);
                double firstX = differences(rest, early), lastX = differences(rest, late);
                assertThat(lastX).isGreaterThan(firstX + 40);
                assertThat(differences(paint(options, BuddyNotice.State.DONE, 400_000_000L),
                    paint(options, BuddyNotice.State.DONE, 1_100_000_000L))).isEqualTo(-1);
                assertThat(differences(rest, paint(options, BuddyNotice.State.RUNNING, 4_500_000_000L))).isEqualTo(-1);
            }
        });
    }
    ...
    @Test void onlyAnActivelyRunningTaskShimmersAndRequestsFrames() {
        ...
        var panel = new BuddyColumnPanel(BuddyTestAppearance.options(), deck, () -> {}, () -> {});
        panel.setClock(() -> 0);
        ...
        deck.orphan(new BuddyNoticeId("test", "one")); panel.refresh();
