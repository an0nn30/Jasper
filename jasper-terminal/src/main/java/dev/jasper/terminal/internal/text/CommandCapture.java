...
public final class CommandCapture {
    ...
    public static String text(long firstRow, int firstColumn, long lastRow, int width, LongFunction<TerminalRow> lineAt) {
        ...
        for (long row = firstRow; row <= lastRow; row++) {
            ...
            if (line == null) break;
            line.readCells(width, cells, null);
            ...
            for (int column = from; column < width; column++) {
                ...
                if (cell != TerminalRow.CONTINUATION) content.append(cell);
            }
