...
public final class TerminalRefactorMeasurement {
    ...
    private static void measure(String name, String line) throws Exception {
        ...
        try (TerminalSession session = new TerminalSession(input,150,45,10_000)) {
            session.startReading();
            input.feed(line.repeat(10_045) + "DONE");
            Await.until(() -> session.snapshot().lineText(44).contains("DONE"), "measurement fixture loaded");
            SwingUtilities.invokeAndWait(() -> {
                ...
                try {
                    Runnable capture = session::snapshot;
                    Runnable paint = () -> painter.paint(graphics,session.snapshot(),
