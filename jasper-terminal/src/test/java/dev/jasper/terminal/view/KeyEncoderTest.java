...
class KeyEncoderTest {
    ...
    void arrowsUseJediTermEncodingIncludingApplicationMode() throws Exception {
        ...
        TerminalSession session = EmulationFixture.unstarted(connector, 20, 4, 10);
        session.internalAccess().startReading();
        try {
            assertThat(text(mac.pressed(key(KeyEvent.VK_UP, 0), session.internalAccess()::codeForKey))).isEqualTo("\033[A");
            connector.feed("\033[?1h"); // application cursor keys on
            Await.until(() -> "\033OA".equals(text(mac.pressed(key(KeyEvent.VK_UP, 0), session.internalAccess()::codeForKey))), "application-mode arrow");
            assertThat(text(mac.pressed(key(KeyEvent.VK_UP, SHIFT), session.internalAccess()::codeForKey))).isEqualTo("\033[1;2A");
        } finally {
            session.close();
        }
    }
