...
class TerminalAppIntegrationTest {
    ...
    void selectionPresenceDoesNotWaitForBufferOrExtractSelectedText() throws Exception {
        ...
        SessionInspection.BufferProbe buffer = terminalBuffer();
        buffer.lock();
        try {
    ...
    void smallPixelAreasAndDirectResizeKeepPtyAndEmulatorAtTheSameMinimum() throws Exception {
        onEdt(() -> {
            ...
            assertThat(session.rows()).isEqualTo(2);
            session.resize(1, 1);
            assertThat(connector.lastGridSize().columns()).isEqualTo(5);
            assertThat(connector.lastGridSize().rows()).isEqualTo(2);
            assertThat(session.internalAccess().snapshot().width()).isEqualTo(5);
            assertThat(session.internalAccess().snapshot().height()).isEqualTo(2);
        });
    }
    ...
    void clearScrollbackPreservesTheLiveScreen() throws Exception {
        connector.feed("1\r\n2\r\n3\r\n4\r\n5\r\n6");
        Await.until(() -> "6".equals(session.internalAccess().snapshot().lineText(3)), "six lines on a four-row screen");
        assertThat(session.internalAccess().snapshot().historyLines()).isPositive();

        view.clearScrollback();

        assertThat(session.internalAccess().snapshot().historyLines()).isZero();
        assertThat(session.internalAccess().snapshot().lineText(3)).isEqualTo("6");
    ...
    void enteringTheAlternateScreenInvalidatesSelectionFindAndCount() throws Exception {
        ...
        show("hello", 0, "hello");
        selectByDragging(0, 0, 4, 0, 0);
        assertThat(view.find(new SearchQuery("hello", false, false)).count()).isOne();
    ...
    void alternateBufferTransitionWhileDetachedIsReconciledOnReattach() throws Exception {
        ...
        connector.feed("\033[?1049h");
        Await.until(session.internalAccess()::usingAlternateBuffer, "alternate screen while detached");
    ...
    void historyResetWhileDetachedIsReconciledOnReattach() throws Exception {
        ...
        assertThat(view.find(new SearchQuery("1", false, false)).count()).isOne();
        onEdt(() -> {
            view.addNotify();
            view.removeNotify();
        });
    ...
    void aClearedAsyncFindCannotRestoreStaleResults() throws Exception {
        ...
        try {
            onEdt(() -> view.findAsync(new SearchQuery("alpha", false, false), result -> callbackRan.set(true)));
    ...
    void repeatedTemporaryDetachesKeepBlockedSearchWorkerAllocationBounded() throws Exception {
        ...
        try {
            onEdt(view::addNotify);
            for (int i = 0; i < 4; i++) {
                onEdt(() -> view.findAsync(new SearchQuery("blocked", false, false), result -> { }));
    ...
    void onlyTheLatestAsyncFindAppliesAndItsCallbackRunsOnTheEdt() throws Exception {
        ...
        onEdt(() -> {
            view.findAsync(new SearchQuery("alpha", false, false), result -> staleCallbackRan.set(true));
            view.findAsync(new SearchQuery("beta", false, false), result -> {
    ...
    void asyncFindCanBeUsedAgainAfterTemporaryDetach() throws Exception {
        ...
        onEdt(() -> {
            view.addNotify();
            view.findAsync(new SearchQuery("alpha", false, false), result -> detachedCallbackRan.set(true));
            view.removeNotify();
            view.addNotify();
            view.findAsync(new SearchQuery("beta", false, false), afterReattach::set);
        });
        try {
            Await.until(() -> {
                drainEventQueue();
    ...
    private SessionInspection.BufferProbe terminalBuffer() throws ReflectiveOperationException {
        return SessionInspection.probe(session);
    }
