...
class TerminalRenderingLifecycleTest {
    ...
    @Test void hiddenOutputDoesNotRepaintButShowingRendersTheLatestText() throws Exception {
        ...
        Await.until(() -> session.internalAccess().snapshot().lineText(0).equals("hidden-output"), "hidden output parsed");
        onEdt(() -> {
            tick("frameTimer");
            tick("blinkTimer");
            assertThat(repaints).hasValue(0);
        ...
        onEdt(() -> {
            ...
            assertThat(timer("frameTimer").isRunning()).isFalse();
            assertThat(session.internalAccess().snapshot().lineText(0)).isEqualTo("hidden-output");
        });
    }
    ...
    @Test void focusedCursorOnlyBlinksWhileVisibleAndRestoresOnFocusLoss() throws Exception {
        ...
        Await.until(() -> !session.internalAccess().snapshot().cursorVisible(), "cursor hidden by application");
        onEdt(() -> {
            tick("frameTimer");
            repaints.set(0);
            tick("blinkTimer");
        ...
        Await.until(() -> session.internalAccess().snapshot().cursorVisible()
            && new dev.jasper.terminal.internal.text.CursorRequest(dev.jasper.terminal.config.CursorStyle.BLOCK, false).equals(session.internalAccess().snapshot().cursorShape()), "steady cursor");
        onEdt(() -> { tick("frameTimer"); assertThat(timer("blinkTimer").isRunning()).isFalse(); });
        connector.feed("\033[1 q");
        Await.until(() -> new dev.jasper.terminal.internal.text.CursorRequest(dev.jasper.terminal.config.CursorStyle.BLOCK, true).equals(session.internalAccess().snapshot().cursorShape()), "blinking cursor");
    ...
    @Test void outputBurstsCoalesceAndLaterOutputSchedulesAnotherFrame() throws Exception {
        for (String text : new String[] {"a".repeat(200) + "FIRST", "\r\nSECOND"}) {
            onEdt(() -> {
                ...
                try {
                    connector.feed(text);
                    Await.until(() -> session.internalAccess().snapshot().lineText(3).endsWith(text.endsWith("FIRST") ? "FIRST" : "SECOND"),
    ...
    @Test void pendingWrapCursorAtLastColumnStillBlinks() throws Exception {
        ...
        Await.until(() -> session.internalAccess().snapshot().lineText(0).length() == 20, "last column written");
        onEdt(() -> {
            focus(true);
            assertThat(timer("blinkTimer").isRunning()).isTrue();
            assertThat(session.internalAccess().snapshot().cursorColumn()).isEqualTo(20);
    ...
    @Test void scrolledOutCursorStopsBlinkingAndReturningToLiveOutputResumesIt() throws Exception {
        connector.feed("old\r\n2\r\n3\r\n4\r\n5\r\n6");
        Await.until(() -> session.internalAccess().snapshot().lineText(3).equals("6"), "history available");
        onEdt(() -> {
            focus(true);
            assertThat(timer("blinkTimer").isRunning()).isTrue();
            view.find(new SearchQuery("old", false, true));
    ...
    @Test void anOldFramePublicationCannotClaimTheReattachedViewsToken() throws Exception {
        ...
        Await.until(() -> session.internalAccess().snapshot().lineText(0).equals("current output"), "current output parsed");
        onEdt(() -> {
