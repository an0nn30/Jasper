...
class ShellIntegrationSessionTest {
    ...
    private CopyOnWriteArrayList<java.time.Duration> durations;
    ...
    @Test void aPromptWithoutACommandStartMarkCapturesNothing() throws Exception {
        ...
        Await.until(() -> session.internalAccess().promptRows().size() == 2, "two prompts");
    ...
    @Test void aPromptResetsTheCommandStartSoALeftoverMarkCapturesNothing() throws Exception {
        ...
        connector.feed("\033]133;A\007$ \033]133;B\007partial text\r\n"
            ...
            + "\033]133;A\007$ ");
        Await.until(() -> session.internalAccess().promptRows().size() == 3, "three prompt marks");
        assertThat(captured).isEmpty();
    }
    ...
    void onlyPromptStartMarksAreRecordedWithTheirRows() throws Exception {
        ...
        Await.until(() -> session.internalAccess().promptRows().size() == 2, "two prompt marks");
        assertThat(session.internalAccess().promptRows()).containsExactly(0L, 2L);
    }
    ...
    void promptRowsStayWithTheirLinesAsOutputScrolls() throws Exception {
        ...
        Await.until(() -> "end".equals(session.internalAccess().snapshot().lineText(3)), "scrolled output");
        long prompt = session.internalAccess().promptRows().getFirst();
        assertThat(prompt).isZero();
        assertThat(session.internalAccess().lineText(prompt)).isEqualTo("p0");
    }
    ...
    void aWidthChangeForgetsPromptRowsBecauseLinesReflow() throws Exception {
        ...
        Await.until(() -> "end".equals(session.internalAccess().snapshot().lineText(3)), "scrolled output");
        assertThat(session.internalAccess().lineText(session.internalAccess().promptRows().getFirst())).isEqualTo("$ mark");

        session.resize(20, 6); // height only: no reflow, the mark stays
        assertThat(session.internalAccess().lineText(session.internalAccess().promptRows().getFirst())).isEqualTo("$ mark");

        session.resize(30, 6);
        assertThat(session.internalAccess().promptRows()).isEmpty();
    }
    ...
    void erasingTheScrollbackForgetsPromptsAndTellsListeners() throws Exception {
        ...
        Await.until(() -> session.internalAccess().promptRows().size() == 1, "one prompt mark");
    ...
    void theTerminalVersionQueryLeavesTheConfiguredCursorAlone() throws Exception {
        ...
        Await.until(() -> "ready".equals(session.internalAccess().snapshot().lineText(0)), "text after the query");
    ...
    void customCommandsFromOtherToolsAreIgnored() throws Exception {
        ...
        Await.until(() -> "done".equals(session.internalAccess().snapshot().lineText(0)), "text after the command");
    ...
    @Test void aRepeatedPromptMarkStillDiscardsAHalfStartedCapture() throws Exception {
        ...
        Await.until(() -> session.internalAccess().promptRows().size() == 1, "the redrawn prompt did not add a row");
        assertThat(captured).isEmpty();
    }
    ...
    /** The notification threshold needs to know how long a command took, which only the marks can say. */
    @Test void aCommandReportsHowLongItRan() throws Exception {
        ...
        try (var timed = EmulationFixture.unstarted(timedConnector, 20, 4, 100, clock::get)) {
            ...
            timed.internalAccess().startReading();
            ...
            Await.until(() -> !reported.isEmpty(), "command reported");
            assertThat(reported.getFirst()).isEqualTo(java.time.Duration.ofSeconds(12));
        }
    }

    /** A prompt mark flushing a stale command never saw a C, so it cannot claim a duration. */
    @Test void aCommandFlushedWithoutAStartMarkReportsNoDuration() throws Exception {
        listenForCommands();
        connector.feed("\033]133;A\007$ \033]133;B\007pwd\033]133;C\007/tmp\r\n\033]133;A\007$ ");
