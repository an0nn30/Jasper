...
class TerminalSessionTest {
    ...
    void outputAppearsOnScreenWithZeroBasedCursor() throws Exception {
        ...
        Await.until(() -> session.internalAccess().snapshot().lineText(0).equals("hello"), "hello on row 0");
    ...
    void cursorOnlyMovesAreReportedAsScreenChanges() throws Exception {
        connector.feed("abc");
        Await.until(() -> session.internalAccess().snapshot().cursorColumn() == 3, "cursor after abc");
        AtomicInteger changes = new AtomicInteger();
        session.addListener(new TerminalSessionListener() {
            @Override
            public void screenChanged() {
        ...
        assertThat(session.internalAccess().snapshot().cursorColumn()).isEqualTo(2);
    }
