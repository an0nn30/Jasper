...
class TerminalPainterTest {
    ...
    void aScrollIndicatorShowsOnlyWhenScrolledBack() throws Exception {
        ...
        TerminalSession session = EmulationFixture.unstarted(connector, COLUMNS, ROWS, 10);
        session.internalAccess().startReading();
        try {
            connector.feed("a\r\nb\r\nc\r\nd\r\ne");
            Await.until(() -> "e".equals(session.internalAccess().snapshot().lineText(2)), "two lines of scrollback");

            BufferedImage scrolled = paint(session.internalAccess().snapshot(0), cursorOff());
            BufferedImage live = paint(session.internalAccess().snapshot(), cursorOff());
    ...
    private ScreenSnapshot snapshotAfter(String output, int expectedCursorColumn) throws Exception {
        ...
        try {
            ...
            Await.until(() -> session.internalAccess().snapshot().cursorColumn() == expectedCursorColumn, "cursor at column " + expectedCursorColumn);
            return session.internalAccess().snapshot();
        } finally {
            session.close();
        }
    }
