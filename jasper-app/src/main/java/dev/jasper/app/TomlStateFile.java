...
/** Shared utilities for bounded, strictly decoded TOML state files and atomic writes. */
final class TomlStateFile {
    private TomlStateFile() { }

    /** Bounded, strictly decoded read of a small TOML state file; empty when the file is absent. */
    static Optional<String> readBounded(Path file, int maxBytes, String what) throws IOException {
        byte[] bytes;
    ...
    static void writeAtomically(Path file, String prefix, String text) throws IOException {
