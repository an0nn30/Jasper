...
class DarkPurpleThemeTest {
    ...
    @Test void missingConfigUsesPurpleAndTheNewBuiltinNeedsNoThemeFile() {
        ...
        assertThat(parsed.snapshot().colors().theme()).isEqualTo("jasper-dark-purple");
        assertThat(parsed.snapshot().colors().appearance()).isEqualTo(Appearance.SYSTEM);
        var selected = new ColorsConfig(Appearance.SYSTEM, "jasper-dark-purple");
    ...
    @Test void systemAndManualAppearanceUsePurpleDarkAndTheExistingLightPalette() {
        ...
        assertThat(light.resolve().palette()).isEqualTo(Palette.jasperLight());
    ...
    @Test void explicitClassicSelectionSurvivesAndPairsWithLightUnderSystemAppearance() {
        var colors = new ColorsConfig(Appearance.SYSTEM, "jasper-dark");
        var classic = ThemeState.defaults().configure(colors, Palette.jasperDark());
        assertThat(classic.resolve().chrome().id()).isEqualTo("jasper-dark");
        assertThat(classic.resolve().palette()).isEqualTo(Palette.jasperDark());
        assertThat(classic.systemChanged(BuiltinTheme.LIGHT).resolve().palette()).isEqualTo(Palette.jasperLight());
        assertThat(classic.systemChanged(BuiltinTheme.DARK).resolve().palette()).isEqualTo(Palette.jasperDark());
    }
    ...
    @Test void realChromeAndPendingTerminalSurfacesSwitchTogetherWithoutLeakingClassicColors() throws Exception {
        edt(() -> {
            ...
            assertThat(UIManager.getColor("Jasper.titleBackground")).isEqualTo(new Color(0x07050d));
            assertThat(UIManager.getColor("Jasper.tabUnderline")).isEqualTo(new Color(0xc089ef));
            themes.systemChanged(BuiltinTheme.LIGHT);
            assertThat(owner.toolbar().getBackground()).isEqualTo(Palette.jasperLight().background());
            themes.configure(new ColorsConfig(Appearance.SYSTEM, "jasper-dark"), Palette.jasperDark());
            themes.systemChanged(BuiltinTheme.DARK);
            assertThat(owner.toolbar().getBackground()).isEqualTo(Palette.jasperDark().background());
