...
import static dev.jasper.app.LaunchSettingsTest.snapshot;
...
class ShellLauncherTest {
    ...
    @Test void applicationWindowLauncherFreezesGridButReadsSessionDefaultsForEachRequest() throws Exception {
        ...
        edt(() -> {
            var launcher = JasperApplication.windowLauncher(pending::add, () -> {
            ...
            launcher.launch(directory, (session, failure) -> errors.add(failure));
            JasperApplication.windowLauncher(pending::add, current::get, (path, settings) -> {
