...
final class BenchmarkRun implements AutoCloseable {
    ...
    private MorayApplication application;
    ...
    static void run(BenchmarkOptions options) throws Exception {
        ...
        report.put("schema", "moray-benchmark"); report.put("schemaVersion", 1);
        ...
        Path temporary = Files.createTempDirectory("moray-benchmark-");
    ...
    private static Map<String, Object> environment(BenchmarkOptions options) {
        ...
        env.put("ownership", "MorayApplication / TerminalWindow / WindowContent / TerminalPane / TerminalSession");
        ...
        env.put("fontFamily", dev.moray.terminal.TerminalOptions.defaults().fontFamily()); env.put("fontSize", 16);
    ...
    private void interactive() throws Exception {
        ...
        for (int i = 0; i < options.cycles(); i++) {
            ...
            int count = edt(() -> {
                ...
                for (TerminalPane pane : panes) pane.view().findAsync("moray", false, true, ignored -> completed.incrementAndGet());
                return panes.size();
            });
