...
class CommandPaletteShortcutsTest {
    ...
    void completePaletteSequencesSendZeroBytesToTheRealTerminalConnector(
        ...
        for (boolean mac : new boolean[]{true, false}) {
            ...
            environment.put("MORAY_READY", ready.toString()); environment.put("MORAY_RECEIVED", received.toString());
            ...
            try {
                edt(() -> {
                    var launcher = new ShellLauncher(pending::add, path -> {
                        try {
                            return dev.moray.terminal.TerminalSession.start(java.util.List.of("/bin/sh", "-c",
                                "stty -echo -icanon min 1 time 0; : > \"$MORAY_READY\"; dd bs=1 count=1 of=\"$MORAY_RECEIVED\" 2>/dev/null"),
                ...
                edt(() -> {
                    ...
                    java.util.function.Consumer<KeyEvent> send = event -> {
                        if (!router.dispatch(event)) dev.moray.terminal.TerminalKeyTestSupport.handleKey(view, event);
                    };
                    var calls = new java.util.concurrent.atomic.AtomicInteger();
