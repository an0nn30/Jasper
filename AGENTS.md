...
- `TerminalSession.attach` runs a connection instead of a child process; its transport lives in `internal.transport` (JDK only), writes and resizes only enqueue, and an attached session never reports a local working directory.
